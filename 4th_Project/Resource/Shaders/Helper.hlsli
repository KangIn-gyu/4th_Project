...
float3 VisualizeSpotLightCone(float3 pixelWorldPos, SpotLightData light, float3 baseColor)
{
    ...
    if (cosAngle > cosOuterCone)
    {
        ...
        float borderWidth = 0.05; // 더 얇은 경계선
        
        // 경계선만 표시
        if (edgeFactor > (1.0 - borderWidth))
        {
            // 경계선 색상 및 강도 조절
            float edgeIntensity = smoothstep(1.0 - borderWidth, 1.0, edgeFactor);
            // 발광 효과를 위해 색상 강도를 높임
            visualColor = float3(1.0, 0.3, 0.0) * edgeIntensity * intensityFalloff * 2.0;
        }
    }
