...
DXMath::Vector3 AnimationNode::InterpolatePosition(float& currTime)
{
    ...
    {   // 현재 시간에 가장 가까운 Position Key 찾기
        if (positionKeys[i].time >= currTime)
        {
            float t = (currTime - positionKeys[i - 1].time) / (positionKeys[i].time - positionKeys[i - 1].time);
...
DXMath::Quaternion AnimationNode::InterpolateRotation(float& currTime)
{
    ...
    {
        if (rotationKeys[i].time >= currTime)
        {
            float t = (currTime - rotationKeys[i - 1].time) / (rotationKeys[i].time - rotationKeys[i - 1].time);
...
DXMath::Vector3 AnimationNode::InterpolateScale(float& currTime)
{
    if (scaleKeys.empty()) 
    { 
    ...
    {
        if (scaleKeys[i].time >= currTime)
        {
            float t = (currTime - scaleKeys[i - 1].time) / (scaleKeys[i].time - scaleKeys[i - 1].time);
