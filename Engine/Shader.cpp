...
void Shader::CreateShader(const std::wstring_view _filePath)
{
	...
	auto compileShader = [&dwShaderFlags](const std::wstring_view& filePath, const std::string& shaderModel, 
									      ComPtr<ID3DBlob>& shaderBuffer, ComPtr<ID3DBlob>& errorBuffer) {
		HR_T(D3DCompileFromFile(
			...
			errorBuffer.GetAddressOf()			// 오류 메시지
		)); };
	...
	if (shaderPrefix == L"VS")
	{
		if (extension == L"hlsl")
		{
			compileShader(_filePath, "vs_5_0", VSBlob, errorBuffer);
		}
	...
	else if (shaderPrefix == L"PS")
	{
		if (extension == L"hlsl")
		{
			compileShader(_filePath, "ps_5_0", PSBlob, errorBuffer);
		}
		else if (extension == L"cso")
		{
