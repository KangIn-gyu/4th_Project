	...
	for (auto& it : *data->meshs) // 음... 이 방식으로 하기 싫었지만 시간때문에 그냥 타협함.
	{
		if (owner->GetOwner()->GetObjectType() == Object::ObjectType::Basic)
		{
			...
			{			
				...
				{ "BLENDINDICES", 0, DXGI_FORMAT_R32G32B32A32_UINT,  0, offset.Byte(InputLayout::Layout::Value::BlendIndices), D3D11_INPUT_PER_VERTEX_DATA, 0},
				{ "BLENDWEIGHT",  0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offset.Byte(InputLayout::Layout::Value::BlendWeight),  D3D11_INPUT_PER_VERTEX_DATA, 0}
			};
