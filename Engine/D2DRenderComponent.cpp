	...
	if (drawBitmap != nullptr)
	{
		...
		if (drawBitmap != nullptr)
		{
			...
			if (drawBitmap->isDiamond)
			{
				...
				{ (rect.left + rect.right) / 2, rect.top },   // ?ë‹¨ ê¼?§“??
				{ rect.right, (rect.top + rect.bottom) / 2 }, // ?¤ë¥¸ìª?ê¼?§“??
				{ (rect.left + rect.right) / 2, rect.bottom }, // ?˜ë‹¨ ê¼?§“??
				{ rect.left, (rect.top + rect.bottom) / 2 },  // ?¼ìª½ ê¼?§“??
				{ (rect.left + rect.right) / 2, rect.top }    // ?¤ì‹œ ?ë‹¨ (?«ê¸°)
				};
				for (int i = 0; i < 4; ++i)
				{
