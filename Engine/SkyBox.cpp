...
void SkyBox::SetShaders(ID3D11Device* device, std::wstring vsPath, std::wstring psPath)
{
    ...
    std::vector<char> vsData = LoadShaderFile(vsPath);
    std::vector<char> psData = LoadShaderFile(psPath);

    HR_T(device->CreateVertexShader(vsData.data(), vsData.size(),
        nullptr, m_vertexShader.GetAddressOf()));
    HR_T(device->CreatePixelShader(psData.data(), psData.size(),
        nullptr, m_pixelShader.GetAddressOf()));

    D3D11_INPUT_ELEMENT_DESC layout[] = {
       { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 }
    };
    HR_T(device->CreateInputLayout(layout, ARRAYSIZE(layout), vsData.data(),
